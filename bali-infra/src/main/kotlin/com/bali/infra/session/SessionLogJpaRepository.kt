...
interface SessionLogJpaRepository : JpaRepository<SessionLogJpaEntity, UUID> {
    // 특정 세션에 속한 모든 log
    fun findBySessionId(sessionId: UUID): List<SessionLogJpaEntity>

    // 여러 세션의 log를 한 번에 조회 (목록 조회 시 N+1 방지용 배치 조회)
    fun findBySessionIdIn(sessionIds: List<UUID>): List<SessionLogJpaEntity>
