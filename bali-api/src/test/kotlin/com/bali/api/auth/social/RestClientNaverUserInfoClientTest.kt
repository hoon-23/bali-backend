...
class RestClientNaverUserInfoClientTest {
    ...
    @Test
    fun `네이버 서버 오류(5xx)는 SocialProviderUnavailableException을 던진다`() {
        val builder = RestClient.builder()
        val mockServer = MockRestServiceServer.bindTo(builder).build()
        val restClient = builder.build()
        ...
        assertThrows(SocialProviderUnavailableException::class.java) { client.fetchMe("test-access-token") }
        mockServer.verify()
    }

    @Test
    fun `네이버에 연결하지 못하면 SocialProviderUnavailableException을 던진다`() {
        val builder = RestClient.builder()
        val mockServer = MockRestServiceServer.bindTo(builder).build()
        val restClient = builder.build()
        ...
        mockServer.expect(requestTo("https://openapi.naver.com/v1/nid/me"))
            .andExpect(header("Authorization", "Bearer test-access-token"))
            .andRespond(ResponseCreator { throw IOException("connection refused") })
